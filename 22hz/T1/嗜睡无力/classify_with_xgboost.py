...
data = pd.read_excel(r"D:\document\数学建模\比赛代码与文件\202107培训\22hz\T1\嗜睡无力\嗜睡乏力.xlsx", sheet_name="Sheet1")
print(data.info())
...
plt.savefig(r"D:\document\数学建模\比赛代码与文件\202107培训\22hz\T1\嗜睡无力\importance.png")
plt.show()
y_predict = pd.DataFrame(model.predict(X), columns=["predict"])
final = pd.concat([X, y, y_predict], axis=1)
print(final)
final.to_excel(r'D:\document\数学建模\比赛代码与文件\202107培训\22hz\T1\嗜睡无力\result.xlsx')
