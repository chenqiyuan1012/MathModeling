...
data = pd.read_excel(r"D:\document\数学建模\比赛代码与文件\202107培训\22hz\T1\体动\体动.xlsx", sheet_name="Sheet1")
print(data.info())
...
plt.savefig(r"D:\document\数学建模\比赛代码与文件\202107培训\22hz\T1\体动\importance.png")
plt.show()
y_predict = pd.DataFrame(model.predict(X), columns=["predict"])
final = pd.concat([X, y, y_predict], axis=1)
print(final)
final.to_excel(r'D:\document\数学建模\比赛代码与文件\202107培训\22hz\T1\体动\result.xlsx')
